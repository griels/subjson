...
struct SlowMatcher  {
    ...
    std::vector<bool> exprMatches;
    std::map<int,Value> vars;

        SlowMatcher(std::vector<Expression> exprs ):exprs(exprs),exprMatches(exprs.size()){}:

            template<typename T> std::tuple<void*, Err> resolveFieldParam(FieldExpr expr){
 auto               rootVal = vars[expr.Root];

     auto           curVal = rootVal;

                for (auto entry :expr.Path)
                {
                    if mapVal, ok := curVal.(map[string]
                    interface{});
                    ok {
                            curVal = mapVal[field]
                    } else {
